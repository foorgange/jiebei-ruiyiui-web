...
<script>
import * as echarts from 'echarts'  // 修改这里，直接引入echarts
export default {
    name: "Zysjg",
    props: ['optionData'], // 接收父组件传递的数据
    data() {
        return {
            option: {}
        };
    ...
    mounted() {
        //console.log('Donut3D 组件挂载完毕，接收的数据:', this.optionData);
        this.$nextTick(() => {
            this.init();
        });
    },
    watch: {
        optionData(newVal, oldVal) {
            //console.log('optionData 发生变化:', newVal);
        }
    },
    methods: {
        //初始化构建
        init() {
            //构建3d饼状图
            // 修改这里，使用引入的echarts而不是this.$echarts
            let myChart = echarts.init(document.getElementById('charts'));
            this.option = this.getPie3D(this.optionData, 0.76);
...
.buttomCharts {
    background: center top url("./image/3Dbackground.png") no-repeat;
