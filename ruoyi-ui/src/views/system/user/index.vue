              ...
              <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['system:user:resetPwd', 'system:user:edit']">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item command="handleAuthRole" icon="el-icon-circle-check"
                    v-hasPermi="['system:user:edit']">专家详情</el-dropdown-item>
...
import {exportExperts} from "@/api/expert/experts";
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        password: [
          ...
          { min: 5, max: 20, message: '用户密码长度必须介于 8和 20 之间', trigger: 'blur' }
        ],
        email: [
          {
            type: "email",
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      let info = '';
      if(row.userId != null){
        info = '是否删除姓名为'+userNickName+'的账号信息'
      }
      else{
        info = '是否删除选中用户账号信息'
      }
      this.$modal.confirm(info).then(function() {
